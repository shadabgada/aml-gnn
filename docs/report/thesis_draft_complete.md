...
These three architectures embody a progression of design philosophy, but each carries limitations that bear on its suitability for banking AML and on the design of this study. GCN's degree-normalised mean aggregation is efficient and exploits homophily, yet it is transductive and, as a uniform average over neighbours, is vulnerable to over-smoothing, in which repeated aggregation homogenises node representations, a particular risk in the dense, hub-dominated financial graph characterised in Chapter 4. GAT's learnable attention is well-motivated for AML, where only a few of an account's many counterparties are relevant, but its evidence base is drawn largely from citation and social graphs, its attention is a contested form of explanation (Section 2.3.3), and multi-head attention is memory-prohibitive on graphs of transaction scale, a constraint that materialises in Chapter 4. GraphSAGE's inductive sampling suits the continual creation of new accounts, but its mean aggregation can dilute the signal from the structurally distinctive minority accounts on which AML detection depends. Crucially, none of the three was designed for extreme class imbalance or for temporal transaction streams, and their evidence bases are predominantly non-financial; these gaps motivate the class-imbalance treatment (Section 2.5) and the temporal architectures (Section 2.4), and they justify evaluating all three on a banking benchmark rather than assuming that performance on social or citation graphs transfers. All three have been applied to financial crime detection, as discussed in the following section.
...
The difficulty that extreme imbalance poses is not confined to evaluation and loss weighting; it is compounded by message passing itself. Because laundering accounts are a tiny minority, a minority node is almost always embedded in a neighbourhood dominated by legitimate accounts, so neighbourhood aggregation, whether GCN's degree-normalised mean or GraphSAGE's sampled mean, pulls its representation toward the majority and dilutes the very signal that distinguishes it. Repeated aggregation across layers compounds this through over-smoothing, driving minority and majority representations together. This is why adding graph structure does not automatically help under severe imbalance, as the Chapter 4 results confirm for GraphSAGE: the aggregation that lets message passing exploit relational signal is the same operation that can wash out a rare class. Dou et al. (2020) demonstrated this effect empirically and proposed class-balanced edge sampling as a partial remedy, and Wu et al. (2024) extended the direction with dual-channel convolution and label-aware sampling to address class imbalance jointly with disassortativity; the weighted loss adopted here and the preference for degree-normalised over mean aggregation are among the design responses this challenge demands.
...
The EMA update occurs inside the forward pass, ensuring the message projection MLP receives gradients through the memory update pathway during training. Predictions are always made using the old memory state (before the current batch's update), so that the prediction for a given interaction never draws on information from that same interaction, which would not be available at prediction time in deployment. Predicting from new memory (containing the current batch's messages) would allow the model to read a transaction's own edge features back out of memory, inflating apparent performance in a way that does not transfer to deployment, where only past interactions are known. Requiring both training and evaluation to predict from old memory keeps the two consistent and preserves the temporal causality of the prediction task.

**Gradient clipping.** The pos_weight for TGN's binary cross-entropy loss is computed from the training set as the inverse frequency of the positive class, yielding a value of approximately 1244. With the `pos_weight_mult` parameter set to 0.01, the effective pos_weight is 12.4. This means minority class gradients are 12.4 times larger than majority class gradients during training. When gradient clipping was enabled (default value 1.0), nearly all positive-class gradients exceeded the clipping threshold and were truncated. The model received uniformly clipped gradients for laundering examples, regardless of whether the example was easy or hard, preventing it from learning a discriminative boundary for the minority class. Removing gradient clipping (`grad_clip=0`) avoids this truncation: with clipping disabled, first-epoch validation AUC-ROC rises from 0.794 to 0.934.
...
**Scalability.** How the pipeline scales with data volume was measured directly by training and evaluating the static GCN on chronological prefixes of the dataset, from 20% to 100% of transactions, timing three epochs at each size (Appendix F, Figure F.3). Training time per epoch grows faster than linearly with edge count, from 1.7 seconds at one million edges to 13.5 seconds at five million, so training throughput falls from roughly 420,000 to 264,000 edges per second as the graph grows fivefold. Inference remains inexpensive throughout (about one second over five million test edges) and scales roughly linearly, and peak memory grows sub-linearly, the full graph fitting comfortably within a commodity workstation. The declining throughput is the salient result: full-batch message passing becomes progressively more costly per edge as the graph grows, which is why the batched per-edge processing of TGN and the neighbourhood sampling of GraphSAGE are architecturally better suited to substantially larger deployments.

**Generalisability.** All results are on the HI-Small variant; cross-variant generalisation to the larger IBM AML variants, generated by the same process (Altman et al., 2023), is identified as future work (Section 5.5). Within-dataset generalisation to unseen time periods is demonstrated directly: every model is tested on transactions strictly later than those it was trained on, and TGN's per-slice analysis (Section 4.5.3) shows its per-node memory contributes transferable signal in the early, low-prevalence portion of that future period.
...
This study makes five contributions to the theoretical understanding of GNN-based AML detection.
...
**Correspondence between architecture and laundering typology.** The performance ordering maps onto the FATF typologies each architecture is theoretically equipped to capture, and because those typologies are empirically present in the data (Section 4.1), the mapping is concrete rather than speculative. Fan-in and fan-out are purely relational, a single account linked to many counterparties, and are exactly what a static GCN's neighbourhood aggregation represents, which is why GCN already recovers most of the signal that graph structure alone affords. Structuring and layering are instead inherently sequential, a sum split into many small transfers within a short window, or funds moved through an ordered chain of intermediaries, and leave little trace in a single static view or in twelve coarse snapshots. This is the theoretical reason the snapshot models add nothing over static GCN, while TGN's per-transaction processing and per-node memory, built to represent exactly such ordered behaviour, deliver their advantage specifically in AUC-PR: TGN recovers the temporally-defined positives the other models cannot see. The correspondence between architectural mechanism and typology structure, not raw performance alone, is what makes continuous-time modelling the theoretically appropriate choice for AML rather than merely the empirically better one.
...
This study set out to answer how static and temporal graph neural network architectures compare to conventional machine learning for detecting money laundering in financial transaction networks. The answer, supported by a systematic three-tier evaluation on a standardised public benchmark under a single deployment-realistic protocol, is that continuous-time temporal GNNs with per-node memory achieve the strongest detection performance, but temporal modelling is not automatically beneficial: snapshot-based approaches underperformed the static GCN, demonstrating that temporal information must be modelled at the right granularity to add value. The deeper finding is a correspondence between architecture and laundering behaviour: relational typologies such as fan-in and fan-out are captured by static message passing, whereas the sequential typologies of structuring and layering require the transaction-level, memory-based modelling that only the continuous-time approach provides. Money laundering is both relational and temporal, and the tools built to counter it must address both dimensions. Detection performance, however, is only a precondition for deployment: the same evidence shows that turning these models into operational compliance tools additionally requires explainability adequate to regulatory scrutiny, which none of the architectures provides natively and which this study therefore identifies as the central direction for making graph-based AML detection deployable in practice.
...
This appendix provides the complete training, validation, and test set metrics for the conventional ML baselines (Table F.1), learning curves for the six neural models (Figure F.1), a post-hoc feature-importance analysis of the GCN (Figure F.2), and an empirical scalability study (Figure F.3). The training set metrics indicate how well each model fits the training data; the validation set metrics were used for early stopping and threshold calibration. The gap between training and test performance for the tree ensembles reflects the difficulty of generalising the minority class under extreme imbalance and the distribution shift of the chronological split.
...
**Figure F.3: Pipeline scalability with dataset size.** (`results/curves/scalability.png`) The static GCN trained and evaluated on chronological prefixes of the dataset, from one to five million edges, timing three epochs at each size. Left: training time per epoch (which grows faster than linearly) and inference time (roughly linear). Centre: peak process memory (total resident set, which includes the in-memory dataset, and grows sub-linearly). Right: training throughput, which falls from about 420,000 to 264,000 edges per second as the graph grows, the empirical signature of full-batch message passing becoming costlier per edge at scale. Generated by `experiments/run_scalability.py` (seed 42, CPU).
